...
def get_entry_date():
    ...
    while flag:
        ...
        try:
           datetime.strptime(user_date, "%d/%m/%Y").date() #On this line it said user_dat instead of user_date
        except:
...
def check_age (num_junior):
    ...
    for i in range(int(num_junior)): #the num_junior variable was a string so i casted it to an integer
        flag = True
    
        while flag:
            age = input('Please enter the age for junior ticket {} : '.format(i+1)) #add 1 to i so the tickets start at 1
            ...
            try:
                ...
                if int(age) <= 16: #made it be 16 inclusive because 16 should be junior
